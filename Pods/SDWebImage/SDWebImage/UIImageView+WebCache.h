...
/**
 *  是否需要关闭渐隐渐现效果,默认关闭
 *  如需开启,则设置sd_disableFadeEffect = NO;
 *  备注:若不关闭渐隐渐现效果,并且使用回调方式设置图片,则可以使用sd_setImageWithFadeEffect:设置图片
 */
@property (nonatomic, assign) BOOL sd_disableFadeEffect;

/**
 *  设置带渐隐渐现效果的图片
 *  默认开启渐隐渐现效果后每次都会渐隐渐现，包括列表刷新
 */
- (void)sd_setImageWithFadeEffect:(UIImage *)image;

/**
 *  设置带渐隐渐现效果的图片
 *  当缓存类型为内存缓存时，不会出现渐隐渐现的效果
 */
- (void)sd_setImageWithFadeEffect:(UIImage *)image
